      ...
      <VStack sx={{ alignItems: 'flex-start', gap: 0, height: isForHome ? '100%' : 'unset', width: '100%' }}>
        ...
        >
          <HStack sx={{ flex: 1 }}>
            ...
            <NavLink route={routes.homeRoute}>
              <HStack sx={{ alignItems: 'center', gap: '0.5rem' }}>
                <Image alt='Logo' src='/spa-tools-demo/img/logo.svg' sx={{ height: '3rem' }} />
                <Heading size='xl' sx={{ lineHeight: '100%' }}>
                  Live Demo
                </Heading>
              </HStack>
            </NavLink>
          ...
          {!isSmallScreen && (
            <HStack
              as='ul'
              sx={{ alignContent: 'flex-start', gap: '1.5rem', listStyleType: 'none', ml: '1rem', mr: '1rem' }}
            >
              <NavItems />
            </HStack>
          )}
